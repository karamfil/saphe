...
function obj:activate(idList, trayColor, trayText, showKeys)
    ...
    if trayColor then
        ...
        obj.modal_tray:frame({
            x = cres.w - math.ceil(cres.w / 20),
            y = cres.h - math.ceil(cres.w / 20),
            w = math.ceil(cres.w / 20 / 2),
            h = math.ceil(cres.w / 20 / 2)
        })
        obj.modal_tray[1].fillColor = {hex = trayColor, alpha = 0.7}
        obj.modal_tray[2].text = trayText
        obj.modal_tray:show()
    end
    if showKeys then
